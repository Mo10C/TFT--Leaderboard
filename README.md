...
5. **チーム戦4v4／ダブルアップ**：
   - **🧩 チーム組タブ**（第1試合の左）で各チームの**名前**と**既定メンバー**（4人／2人）を設定。POPをタップ → チームの席をタップで配置（ドラッグも可）
     - **🎲 ランダムでチーム分け**＝ロスターの選手をシャッフルして各チームの既定メンバーへ自動割当／**全クリア**＝全チームの既定メンバーを空に
   - 各試合では卓の枠上部の **▼** でチームを選ぶと、その**既定メンバーを引き継いで**席に流し込む（試合ごとにメンバー入替も自由）
   - **🧩 チーム組を反映**＝この試合の全卓に、割り当たっているチームの既定編成を一括流し込み
   - **🔀 対戦カードをシャッフル**＝その試合の卓割りをランダム組み替え（既定編成も引き継ぐ）
   - チーム名・既定メンバーは管理コンソール（editor.html）の「チーム組」からも編集可
6. **自動組卓（全モード）**：参加者バーの
...
- **Riot API 設定**：Worker URL とリージョンを UI から編集・保存（config.js を編集／再デプロイせずに反映）。保存はこのブラウザに記録され本番ボードにも即反映。空の config.js でもここで設定すれば有効化できる。「config.js の値に戻す」でいつでも復帰。Discordアイコン取得も同じ Worker を使用。※Riot APIキーは Worker 側（Cloudflareのシークレット）で管理し、ここには入力しません
- チーム組（名前＋既定メンバー）の編集
- **データ管理**：
  - **ボード（大会）切替 POP**：保存済みのボードが右側に POP で並び、タップで切替。各 POP から 📊 アーカイブ表示 ／ ✏️ 表示名の変更 ／ 🗑 一覧から除外（データは残る）
  - **大会名（表示名）**：この大会に名前を付けて変更可能（POP や見出しに反映）
  - **アーカイブ閲覧**：📊 で、その大会の **総合結果**（個人／チーム通算）と **各試合の卓別結果** を読み取り専用で表示（別ボードも切替なしで閲覧可）
  - JSON バックアップ／復元・ボードID指定オープン（新規作成も）
- 順位の手入力／修正
- 接続テスト・全卓一括取得
- JSONバックアップ／復元
...
> チーム／ダブルアップの「チーム名」と「既定メンバー」は本番ボードの🧩チーム組タブ（または管理コンソール）で編集。
> チームは名前＋ID＋既定メンバーを持つ永続エンティティで、各試合に卓のどのサイド／枠へ座るか（対戦カード）を選ぶと既定メンバーを引き継ぎます。
> メンバーは試合ごとに入替可。総合順位はチームIDで全試合を通算するため、試合ごとにメンバーが入れ替わっても正しく積み上がります。
